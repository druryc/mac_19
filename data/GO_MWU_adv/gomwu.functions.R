...
gomwuStats=function(input,goDatabase,goAnnotations, goDivision, Module=FALSE, Alternative="t", adjust.multcomp="BH", clusterCutHeight=0.25,largest=0.1,smallest=5,perlPath="perl", shuffle.reps=20){
	...
	message(sum(fdr<0.05)," GO terms at 5% FDR")
	message(sum(fdr<0.01)," GO terms at 1% FDR")
	#message(" endline",cat((sum(fdr<0.1)-sum(fdr<0.05)),(sum(fdr<0.05)-sum(fdr<0.01)),sum(fdr<0.01),sum(fdr<0.1)))
...
gomwuPlot=function(inFile,goAnnotations,goDivision,level1=0.1,level2=0.05,level3=0.01,absValue=-log(0.05,10),adjusted=TRUE,txtsize=1,font.family="sans",treeHeight=0.5,colors=NULL) {
	...
	text(left,top-step*4,paste("p < ",level2,sep=""),font=1,cex=0.8* txtsize,adj=c(0,0),family=font.family)
	text(left,top-step*6,paste("p < ",10^(-cutoff),sep=""),font=3,col="grey50",cex=0.8* txtsize,adj=c(0,0),family=font.family)
